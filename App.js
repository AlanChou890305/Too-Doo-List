...
export default function App() {
  ...
  // Removed dynamic taskAreaHeight for full bottom fill
  ...
  const goToNextMonth = () => {
    ...
    // Try to keep the same day if possible, otherwise fallback to last day of month
    ...
    setSelectedDate(`${newYear}-${String(newMonth + 1).padStart(2, "0")}-${String(safeDay).padStart(2, "0")}`);
  };
  ...
  // Handle vertical swipe gesture for month navigation
  const handleVerticalGesture = ({ nativeEvent }) => {
    console.log('Vertical gesture detected:', nativeEvent.translationY, 'state:', nativeEvent.state);
...
// --- Only one version of each function is kept below ---
...
const renderTaskArea = () => {
  return (
    <PanGestureHandler onHandlerStateChange={handleTaskAreaSwipe}>
      <Animated.View style={styles.taskArea}>
        <View style={styles.taskAreaContent}>
          ...
          <ScrollView
            ...
            horizontal={false}
          >
            {/* Render tasks here, e.g., FlatList or map */}
          </ScrollView>
        </View>
      </Animated.View>
    </PanGestureHandler>
